...
WebSocketManager.subscribe("chat_message", async (userId, data) => {
  ...
  if (lobby) {
    ...
    if (data.message.toLowerCase().trim() === lobby.currentWord.toLowerCase().trim()) {
      ...
      if (lobby.correctGuesses.length >= lobby.players.length - 1) {
        // All except drawer
        // Find and call the endRound function from lobbyService
        const lobbyService = require("./lobbyService")
        if (typeof lobbyService.endRound === "function") {
          lobbyService.endRound(lobby.lobbyId)
        }
      }
