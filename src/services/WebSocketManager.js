...
class WebSocketManager {
  ...
  // Entfernt eine bestimmte Callback-Funktion von einem Event-Typ
  static unsubscribe(eventType, callback) {
    if (WebSocketManager.subscribers.has(eventType)) {
      const callbacks = WebSocketManager.subscribers.get(eventType);
      const updatedCallbacks = callbacks.filter((cb) => cb !== callback);

      if (updatedCallbacks.length > 0) {
        WebSocketManager.subscribers.set(eventType, updatedCallbacks);
      } else {
        WebSocketManager.subscribers.delete(eventType);
      }
